...
(function (global) {
  ...
  var MEMBER_CONFIG = {
    ...
    // ─────────────────────────────────────────────
    qaList: [
      { q: '等级体系介绍',           a: '<p>唯普会员等级<strong>每月 1 日统一定级</strong>，由用户最近 3 个自然月的累计成交台数与当前账户保证金(需 ≥ 2000 元)共同决定。当月内成交或保证金的变化，将在<strong>下月 1 日</strong>定级时生效，当月等级保持不变。</p><p>每个等级可享受不同类型的权益，等级越高权益越丰富。</p>' },
      { q: '什么是成交台数?',        a: '<p>成交台数是用户最近 3 个自然月(含本月)在唯普平台<strong>已付款</strong>的车辆数量，且<strong>不含退车</strong>。</p><p>特别提示：仲裁退车将冲减对应的成交台数。</p>' },
      { q: '会员等级变化规则',   a: '<p>会员等级<strong>每月 1 日统一定级</strong>，依据用户最近 3 个自然月的累计成交台数与当前账户保证金(需 ≥ 2000 元)评定，定级结果在当月内保持不变。</p><p><strong>升级：</strong>当月中即使成交台数已达到更高等级门槛，等级也不会立即变动，将于<strong>下月 1 日</strong>统一升级；在此之前，会员页面会提前显示「已满足 V× 条件 · 下月 1 日自动升级」。</p><p><strong>降级：</strong>若账户保证金低于 2000 元或成交台数不足，同样在下月 1 日统一下调。</p>' },
  ...
  // 入参 lockedShort=已生效等级(上月 1 日定级结果) / deals=近 3 月成交台数(含本月) / depositOk=保证金是否≥2000
  ...
  MEMBER_CONFIG.getLevelProgress = function (lockedShort, deals, depositOk) {
    ...
    var dealsLevel = MEMBER_CONFIG.getLevelByDeals(deals);          // 仅按台数(忽略保证金)的应得等级
