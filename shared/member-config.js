...
(function (global) {
  ...
  var MEMBER_CONFIG = {
    ...
    levels: [
      {
        ...
        merchantCount: 14438,
        // 该等级享有的会员权益(以会员权益为主,唯金币为辅)
        benefits: [
          { id: 'coin_exchange_100', icon: 'coins', label: '唯金币 1:1 兑换', desc: '按 1 金币 = 0.093 元等值兑换权益卡' }
        ]
      },
      {
        code: 'LV1',
        ...
        merchantCount: 499,
        benefits: [
          { id: 'coin_exchange_100', icon: 'coins', label: '唯金币 1:1 兑换', desc: '按 1 金币 = 0.093 元等值兑换' },
      ...
      },
      {
        ...
        merchantCount: 310,
        benefits: [
          { id: 'coin_exchange_95', icon: 'tag', label: '唯金币 95 折兑换', desc: '兑换权益卡时享 95 折' },
      ...
      },
      {
        ...
        merchantCount: 121,
        benefits: [
          { id: 'coin_exchange_80', icon: 'tag', label: '唯金币 8 折兑换', desc: '兑换权益卡时享 8 折' },
          { id: 'service_pack', icon: 'package', label: '服务中心权益包', desc: '停车费减免、物流、代驾等上门服务' },
          { id: 'refund_1', icon: 'undo-2', label: '无偿退车 1 台', desc: '每定级周期可享 1 台无偿退车' },
          { id: 'history_data', icon: 'scroll-text', label: '历史数据查询', desc: '查询车源历史成交数据' },
          { id: 'reserve_price', icon: 'target', label: '保留价查询', desc: '查询车辆保留底价' },
    ...
    // ─────────────────────────────────────────────
    // 二、唯金币(积分)规则 · 根据商户行为动态赋值
    // ─────────────────────────────────────────────
    coin: {
      unitYuan: 0.093,                 // 一个金币 = 0.093 元
    ...
    coinEarning: [
      {
        ...
        icon: 'wallet',
        items: [
          { id: 'deal', action: '完成车辆交易', coin: 50, unit: '金币', desc: '成交一台车可获得', icon: 'wallet' },
          { id: 'bid', action: '有效出价参与竞拍', coin: 10, unit: '金币/台', desc: '出价 1 台车 1 次,不封顶', icon: 'gavel' }
        ]
      },
      {
        category: '活跃互动类(低门槛)',
        ...
        icon: 'message-circle',
        items: [
          { id: 'vote', action: '参与平台投票 / 问卷 / 调研', coin: '1-5', unit: '金币/次', desc: '视当期互动方案,回馈平台商户', icon: 'clipboard-list' },
          { id: 'bank_verify', action: '完善账户银行卡信息,并充值保证金', coin: '1-5', unit: '金币/次', desc: '一次性奖励,仅首次完善有效', icon: 'credit-card' },
          { id: 'biz_verify', action: '上传营业执照并完成实名认证(解锁企业车商特权)', coin: '1-5', unit: '金币/次', desc: '一次性奖励,仅首次完善有效', icon: 'badge-check' }
        ]
      },
      {
        category: '社交裂变类(高积分)',
        ...
        icon: 'share-2',
        items: [
          { id: 'share', action: '转发车源并成功分享朋友圈', coin: 1, unit: '金币', desc: '', icon: 'megaphone' },
          { id: 'invite_member', action: '邀请好友注册并开通会员', coin: 2, unit: '金币', desc: '双方都有金币增送,好友需完成实名认证 + 保证金充值', icon: 'user-plus' },
          { id: 'refer_deal', action: '推荐好友完成首笔交易', coin: 2, unit: '金币', desc: '好友需在 30 天内通过平台完成有效成交,按成交日核算', icon: 'users' }
        ]
      },
      {
        category: '限时活动类(不定期)',
        ...
        icon: 'calendar-heart',
        items: [
          { id: 'festival', action: '节日专属任务(如中秋 / 春节)', coin: 1, unit: '金币', desc: '以具体活动通知为准', icon: 'gift' },
          { id: 'weekend_bid', action: '周六日出价可获双倍积分', coin: 'x2', unit: '按出价金币算', desc: '以具体活动通知为准', icon: 'zap' }
        ]
      },
      {
        category: '增值服务类',
        ...
        icon: 'wrench',
        items: [
          { id: 'weichezhuan', action: '开通 / 使用唯车转,并成为会员', coin: 1, unit: '金币', desc: '完成开通 / 使用且会员激活后发放', icon: 'repeat' },
          { id: 'value_service', action: '使用物流 / 代驾 / 复检 / 购专业检测等服务', coin: 5, unit: '金币/次', desc: '按实际使用台次结算', icon: 'truck' },
          { id: 'agency_service', action: '使用平台代办服务(过户 / 年审 / 续保 / 配件)', coin: 5, unit: '金币/次', desc: '按实际使用台次结算', icon: 'clipboard-check' }
        ]
      }
    ],
    ...
    redeemableRewards: [
      ...
      { id: 'r_license',       name: '平台行驶证查询卡',   coin: 100, scope: '所有等级商户', icon: 'file-text' },
      { id: 'r_crash_report',  name: '碰撞报告查询卡',     coin: 200, scope: '所有等级商户', icon: 'car-front' },
      { id: 'r_transfer_fee',  name: '过户费减免 100 元',  coin: 500, scope: '所有等级商户', icon: 'percent' }
    ]
  };
