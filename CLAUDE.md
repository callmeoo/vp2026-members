...
- **等级定级时机(月度统一定级 · 重要)**: 会员等级**每月 1 日统一定级**,依据近 3 个自然月累计成交台数(**含本月滚动**,如 6 月看 4-6 月;展示数随成交实时累加,等级仅每月 1 日对该数快照——故进度条实时动但升降级次月生效) + 当前账户保证金(需 ≥ 2000)评定,**当月内锁定不变**;月中成交 / 保证金变化一律**下月 1 日生效,不实时升降**(原 Q&A "实时升级"口径已废弃)。`shared/member-config.js → getLevelProgress(lockedShort, deals, depositOk)` 是 app/pc 两端共用状态机,返回 5 态:`growing`(还差 X 台升 V×) / `pending`(已达更高门槛待生效,展示「已满足 V× 条件 · 下月 1 日自动升级」) / `deposit-low`(保证金<2000 压在 V0,引导「充至 2000 参与定级」,台数已够则补「达标后可升 V×」) / `downgrade-warn`(保证金跌破 2000,V1-V3 预警下月下调) / `max`(V3 已最高)。关键:**已生效等级与实时台数解耦**——hero 大字 = 上月定级结果(`lockedShort`),不随当月台数实时变;`deposit-low`/`downgrade-warn` 进度条置灰。
...
| 改等级进度文案 / 定级状态机(月度统一定级) | `shared/member-config.js → getLevelProgress`(5 态:growing/pending/deposit-low/downgrade-warn/max) + `app/member.html` / `pc/member.html` 进度区 |
